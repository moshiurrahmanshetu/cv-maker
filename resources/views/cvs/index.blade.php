...
@section('title', 'My CVs - CV Maker')

@section('content')
<div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 mb-4">
    <div>
        <h1 class="h3 fw-bold mb-1">My CVs & Resumes</h1>
        <p class="text-muted small mb-0">Create, edit, duplicate, and manage all your career documents.</p>
    </div>
    ...
    <form method="GET" action="{{ route('cvs.index') }}" class="row g-2 align-items-center">
        <div class="col-md-6">
            <div class="input-group">
                ...
                <input type="text" name="search" class="form-control border-start-0" placeholder="Search by title or summary..." value="{{ request('search') }}">
            </div>
        </div>
...
@if($cvs->isEmpty())
    <div class="card card-saas text-center py-5">
        ...
        <h3 class="h5 fw-bold mb-1">No CVs found</h3>
        <p class="text-muted small mb-4">
            @if(request('search') || request('status'))
                No resumes match your current filter criteria.
            @else
                ...
                <div class="card card-saas h-100 p-4 d-flex flex-column justify-content-between">
                    <div>
                        <div class="d-flex align-items-start justify-content-between gap-2 mb-2">
                            <h2 class="h6 fw-bold mb-0 text-truncate" title="{{ $cv->title }}">
                                {{ $cv->title }}
                            </h2>
                            @if($cv->isPublished())
                    ...
                    <div class="pt-3 border-top">
                        ...
                        <div class="d-flex align-items-center justify-content-between gap-1">
                            <div class="d-flex gap-1">
                                <a href="{{ route('cvs.show', $cv) }}" class="btn btn-sm btn-saas-secondary py-1 px-2" title="Preview CV">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <a href="{{ route('cvs.edit', $cv) }}" class="btn btn-sm btn-saas-primary py-1 px-2" title="{{ $cv->isDraft() ? 'Continue Draft' : 'Edit CV' }}">
                                    ...
                                    <li>
                                        <form action="{{ route('cvs.duplicate', $cv) }}" method="POST">
                                            ...
                                            <button type="submit" class="dropdown-item dropdown-item-saas">
                                                <i class="bi bi-copy"></i> Duplicate CV
                                            </button>
                                        </form>
                                    </li>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content border card-saas">
                        <div class="modal-header border-bottom">
                            <h5 class="modal-title fw-bold fs-6" id="deleteModalLabel{{ $cv->id }}">Delete CV</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body py-4">
                            <p class="mb-1">Are you sure you want to permanently delete <strong>"{{ $cv->title }}"</strong>?</p>
                            <p class="small text-muted mb-0">This action will remove all associated experiences, education, and skills. This cannot be undone.</p>
                        </div>
